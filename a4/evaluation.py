...
class GroupEvaluator(object):
    ...
    def get_group_scores(self):
        ...
        for i in range(len(gl)):
            ...
            #np.median(y_hat[frm:to])
            #scores[i] = (np.min(y_hat[frm:to])+np.max(y_hat[frm:to]))/2
            frm = to
        return scores
...
class InstanceEvaluator(object):
    ...
    def load_labeled_instances(self, filename, separator=' ', quiet=False):
        ...
        with open(filename) as f:
            for line in f:
                elements = line.split('\t')
                score = float(elements[len(elements) - 1][1])
                #if score!=3:
                 #   score=1;
                #else:
                #    score=0;
                if score == 0:
                    ...
                    pos += 1
                features = np.fromstring(elements[len(elements) - 2], dtype=float, sep=separator)
