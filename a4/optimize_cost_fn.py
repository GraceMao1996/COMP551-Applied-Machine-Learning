...
class GICF(object):
    ...
    def set_parameters(self, batch_size=500, alpha_balance=0.04, lr=0.1, momentum_value=0.7, similarity_fn='rbf',
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            while X is not None:  # for each mini-batch # do gd step
                ...
                # new theta
                #
                theta = self.momentum_value * theta -  self.lr / (epoch + 1) * theta_der
    ...
    def _print_progress(self, theta, print_details=False):
        ...
        print (round(100 * acc_train, 2), ' \t\t(', round(100 * auc_train, 2), ')\t','acc_train',)

        acc, auc = self.test_eval.evaluate_groups(theta, print_details)
        self.group_acc.append(acc)
        accback=acc
        print (round(100 * acc, 2), ' \t\t(', round(100 * auc, 2), ')\t','evagroup',)
        print ('\t|\t',)
        ...
        if print_details:
            ...
            plt.show()
        print(Jilu)
        print (round(100 * acc, 2), '\t\t(', round(100 * auc, 2), ')\t', ' \t(', round(100 * auprc, 2), ')\t','evainstance',)
