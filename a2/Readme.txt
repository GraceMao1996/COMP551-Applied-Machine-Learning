To read raw txt files into .csv files, please run Read.py first.
...
Simply run SVM.py with train.csv and test.csv in the same folder. for different parameter, you can choose ngram, C by yourself
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------

Required Library for Doc2vec
numpy
pandas
re,string
sklearn.feature_extraction.text --TfidfVectorizer
...
Put train.csv and test.csv into the folder and simply run Doc2vec.py.
For the first time to run Doc2vec, you need to get the Voculabory wich requires a 4 core cpu with 8 threads and wait around 20 min to get the vocabulabary. 
it has logistic regression and linearSVM mode by call function LG() or LSVM(). LG has penalty and C. LinearSVM has parameter C
